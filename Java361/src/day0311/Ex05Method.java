...
// 메서드는 선언과 구현으로 이루어진다.
// 메서드의 선언이란 해당 메서드가 왜부 클래스에서 접근 가능한지, static인지, 메서드가 종료될 때
// 그 메서드를 실행시킨 곳으로 어떤 데이터타입의 값을 보내줄지, 이름은 무엇인지, 그 메서드를 실행시킬 때
// 외부에서 어떤 값을 보내줄지를 지정하는 것이다.

// 또한 해당 메서드를 선언하고 그 안에 그 메서드를 실행시켰을 떄 실행할 코드를 우리가 구현하게 된다.
...
// 우리가 필요에 따라서 다양한 접근제한자를 사용할 수 있지만
...
// 리턴타입: 리턴타입이란, 해당 메서드가 종료될 때, 그 메서드를 실행시킨 곳으로 보내줄 값의
//         데이터타입을 적어주게 된다.
//         만약 아무런 값도 다시 돌려주지 않을 경우, 우리가 void 라고 적는다.
//         리턴 타입이 존재할 시에는, 해당 메서드를 구현할 떄 반드시 "return" 이라는 키워드를 사용해서
...
public class Ex05Method {
    ...
    // 2개의 int 값에 대한 산술연산자 결과값을 이쁘게 출력하는 printCalcResult()
    // 이 메서드의 리턴 타입은? void
