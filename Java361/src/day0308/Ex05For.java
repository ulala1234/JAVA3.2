...
public class Ex05For {
    public static void main(String[] args) {
        ...
        // 1. int i = 1; 이 실행되서 i 의 현재 값이 1이 된다.
        // 2. i <= 4; 가 실행되는데 i의 현재 값이 1이므로 true가 나온다.
        // 3. 조건식이 true가 나왔으므로 반복할 코드인 System.out.println(i)가 실행되서
        //    화면에 1이 출력된다.
        // 4. 더이상 반복시킬 코드가 없으므로 변화식인 i++이 실행되서 i의 현재 값이 2가 된다.
        // 5. i <= 4; 가 실행되는데 i의 현재 값이 2이므로 true가 나온다.
        // 6. 조건식이 true가 나왔으므로 반복할 코드인 System.out.println(i)가 실행되서
        //    화면에 2가 출력된다.
        // 7. 더이상 반복시킬 코드가 없으므로 변화식인 i++이 실행되서 i의 현재 값이 3이 된다.
        // 8. i <= 4; 가 실행되는데 i의 현재 값이 3이므로 true가 나온다.
        // 9. 조건식이 true가 나왔으므로 반복할 코드인 System.out.println(i)가 실행되서
        //    화면에 3이 출력된다.
        // 10. 더이상 반복시킬 코드가 없으므로 변화식인 i++이 실행되서 i의 현재 값이 4가 된다.
        // 11. i <= 4; 가 실행되는데 i의 현재 값이 4이므로 true가 나온다.
        // 12. 조건식이 true가 나왔으므로 반복할 코드인 System.out.println(i)가 실행되서
        //    화면에 4가 출력된다.
        // 13. 더이상 반복시킬 코드가 없으므로 변화식인 i++이 실행되서 i의 현재 값이 5가 된다.
        ...
        // 2. 제어 변수는 정말로 변수이다. 다만 유효 범위는 해당 for문 안이 된다.
        for(int i = 1; i <= 3; i++) {
            System.out.println(i);
        }
        
        // 3. 만약 처음부터 반복조건식이 false가 나오면
        ...
        //    분리시킬수도 있지만 이 방법은 최대한 사용을 자제하는 것이 좋다.
